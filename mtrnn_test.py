...
class RNN(chainer.Chain):
        ...
        def __init__(self, io_size, fast_hidden_size, slow_hidden_size):
                super(RNN, self).__init__()
                with self.init_scope():
                        self.x_to_fh = L.Linear(io_size, fast_hidden_size)
                        self.fh_to_fh = L.Linear(fast_hidden_size, fast_hidden_size)
                        self.fh_to_sh = L.Linear(fast_hidden_size, slow_hidden_size)
                        self.sh_to_fh = L.Linear(slow_hidden_size, fast_hidden_size)
                        self.sh_to_sh = L.Linear(slow_hidden_size, slow_hidden_size)
                        self.fh_to_y = L.Linear(fast_hidden_size, io_size)
...
#     model = chainer.ChainList(x_to_fh=L.Linear(io_size, fast_hidden_size),
#                             fh_to_fh=L.Linear(fast_hidden_size, fast_hidden_size),
#                             fh_to_sh=L.Linear(fast_hidden_size, slow_hidden_size),
#                             sh_to_fh=L.Linear(slow_hidden_size, fast_hidden_size),
#                             sh_to_sh=L.Linear(slow_hidden_size, slow_hidden_size),
#                             fh_to_y=L.Linear(fast_hidden_size, io_size))
#     for param in model.parameters:
#         param[:] = np.random.uniform(-0.1, 0.1, param.shape)
#     return model
...
for epoch in range(1,args.epoch+1):
    ...
    for i in six.moves.range(0, N, args.batchsize): #i jumps 0->batchsize->2*batchsize->...->k*batchsize < N
        ...
        acc_loss.backward()
        optimizer.update()
        #optimizer.hyperparam.weight_decay_rate(args.weightdecay)
